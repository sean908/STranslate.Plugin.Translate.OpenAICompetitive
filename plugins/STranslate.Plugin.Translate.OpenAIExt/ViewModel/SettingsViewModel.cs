...
public partial class SettingsViewModel : ObservableObject, IDisposable
{
    ...
    [ObservableProperty] public partial string FetchModelsResult { get; set; } = string.Empty;
    ...
    private async Task FetchModelsAsync()
    {
        try
        {
            FetchModelsResult = string.Empty;

            if (string.IsNullOrWhiteSpace(Url))
                throw new Exception("请先填写 API URL");
            ...
            var response = await _context.HttpService.GetAsync(BuildModelsUrl(Url), option);
            var root = JsonNode.Parse(response);
            var modelIds = root?["data"]?.AsArray()
            ...
            FetchModelsResult = $"已获取 {Models.Count} 个模型";
            _context.Snackbar.ShowSuccess(FetchModelsResult);
        }
        catch (Exception ex)
        {
            FetchModelsResult = "获取模型失败";
            _context.Logger.LogError(ex, FetchModelsResult);
            _context.Snackbar.ShowError($"{FetchModelsResult}: {ex.Message}");
        }
    }
